      ...
      {#each TemplateList as template, i}
        <li class="list-group-item bg-transparent liSpecial">
          <ListItem
            id={i}
            content={template}
            close_function={RemoveFromList}
            repeat_function={RepeatEntry}
          />
        </li>
      {/each}
      <li class="list-group-item bg-transparent liSpecial">
        <button
          ...
          data-bs-target="#FilterModal">Add</button
        >
      </li>
    </ul>
  </div>
...
<Filters bind:AppObjects StoreFunction={AddToList} />
