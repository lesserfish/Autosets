...
function ReplaceEntry()
{
  ...
  <button class="btn bi-arrow-repeat col" type="button" style="font-size:130%" on:click={ReplaceEntry}></button>
  <a href="{ExerciseEntry["Link"]}" class="link-primary" target="_blank" rel="noopener noreferrer"> Link </a>

<div class="collapse" id="collapseLink{id}">
  <div class="card card-body bg-transparent cardSpecial">
    <b> Muscle groups:</b> <p>{ExerciseEntry["MuscleGroup"]}</p>
    <b> Equipment:</b> <p>{ExerciseEntry["Equipment"]}</p>
    <b> Difficulty:</b> <p>{ExerciseEntry["Experience"]}</p>
    <b> Exercise Type:</b> <p>{ExerciseEntry["Type"]}</p>
  </div>
</div>
